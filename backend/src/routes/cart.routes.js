...
routes.get("/", auth, async (req, res) => {
  const cartFetch = await cart
    ...
    .populate("products.product", "title _id stock price images");
...
routes.delete("/:id", auth, async (req, res) => {
  ...
  const updatedProducts = findcart.products.filter(
    (item) => item.product.toString() !== id
  );

  findcart.products = updatedProducts;

  let total = 0;
  for (let element of findcart.products) {
    const prod = await product.findById(element.product);
    total += element.quantity * prod.price;
  }

  findcart.totalvalue = total;
  await findcart.save();
